...
from parcing import get_weather_by_ip
from parcing import get_datetime
from parcing import get_news_from_lenta_working
...
def test_get_weather_by_ip_1():
    ...
    with patch('parcing.requests.get', side_effect=[mock_response, mock_response]), \
...
def test_get_datetime1():
    ...
    with patch('parcing.requests.get', return_value=mock_response):
        try:
            ...
            # В данном тесте мы просто проверяем, что функция не падает с критичными ошибками
            # (но она может падать с ожидаемыми исключениями при некорректных данных)
            pass
...
def test_get_news_from_lenta_working2():
    ...
    with patch('parcing.requests.get', return_value=mock_response), \
         ...
         patch('parcing.re.sub', side_effect=lambda pattern, repl, string: string):
